...
from quantum_processing.hamiltonian_builder import hamiltonian_builder
...
def build_patch_records(nodes, patches):
    ...
    for p in patches:
        interior_nodes = nodes[p["interior_idx"]]
...
@task(concurrency_limit="hamiltonian-medium")
def build_hamiltonian_task(record: PatchRecord):
    H = hamiltonian_builder(
        phi=[0.0] * len(record.patch_nodes),
